use crate::bytecode_types::{ByteCode, CodeError, Codes, Operations, Result, Variable};
...
macro_rules! perform_loop_op {
    ($expression: expr, $op: tt) => {{
                ...
                let x = $expression.stack[size-1];
                let y = $expression.stack.remove(size-2);
...
pub fn interpret(bytecodes: Vec<ByteCode>) -> Result<Variable> {
    ...
    for bytecode in bytes.bytecodes {
        if let Some(err) = match bytecode {
            ...
            ByteCode::ReadVar(c) => {
                let read_value = bytes
                    .stack
                    .iter()
                    .find(|&&item| item.variable == Some(c))
                    .unwrap();
                bytes.stack.push(Variable {
